...
clc;clear all;
cd 'D:\CPR Research\Research_collaboration\Work\Native\Coffeelake';
...
samples=4000;
num_measure=100;
len_per_file=samples*num_measure;

for k=1:length(Files)
   data=rmmissing(readmatrix(strcat('Data/',(Files(k).name))));
   image(:,k)=data(1:len_per_file,2);
end

%% Segment measurment-wise
num_image=length(Files);
...
%% Save
cd 'D:\CPR Research\Research_collaboration\Work\Native\Coffeelake\Final';
